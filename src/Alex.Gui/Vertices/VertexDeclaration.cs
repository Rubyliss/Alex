...
namespace Alex.Engine.Vertices
{
	public class VertexDeclaration : IEquatable<VertexDeclaration>
	{
		...
		internal VertexLayoutDescription LayoutDescription
		{
			get
			{
				var ourData = InternalVertexElements.ToArray();

				VertexElementDescription[] data = new VertexElementDescription[ourData.Length];
				for (var index = 0; index < InternalVertexElements.Length; index++)
				{
					var element = InternalVertexElements[index];
					VertexElementFormat format = element.VertexElementFormat;

					VertexElementDescription description = new VertexElementDescription();
					description.Format = format;
					description.Semantic = element.VertexElementUsage;

					data[index] = description;
				}


				return new VertexLayoutDescription(data);
			}
		}
		...
		private static int GetVertexStride(VertexElement[] elements)
		{
			...
			{
				var start = elements[i].Offset + FormatHelpers.GetSizeInBytes(elements[i].VertexElementFormat);
				if (max < start)
					max = (int) start;
			}
